...
namespace StackTestT9
{
    class Program
    {
        ...
        //PILA LINEAL
        // aca basicamente es el mismo metodo que se vio en clase solamente que aca se incluye el metodo eliminarcaracteresespeciales
        ...
        static void Expresiones_Matematicas()
        {
            ...
            int n = 0, a = 0, b = 0, r = 0;
            Console.WriteLine("Ingrese la expresión en prefix");//prefix sig. igresar el simbolo de operacion antes y luego los numeros a usar/82
            string exp = Console.ReadLine();                    //infix es como normalmente lo hacemos por ejemplo 8/2
            char caracter = ' ';                                //postfix se usa el signo por ultimo 82/
            for (n = exp.Length - 1; n >= 0; n--)
            {
                ...
                else
                {
                    a = miPila.Pop();
                    b = miPila.Pop();
                    //reconocer que signo vamos a usar (if)
                    if (caracter == '+')
                    {
                        r = a + b;
                        miPila.Push(r);
        ...
        // LIFO el ultimo que entra primero en salir
